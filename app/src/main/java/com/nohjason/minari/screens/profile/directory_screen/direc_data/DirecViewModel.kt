...
class DirecViewModel @Inject constructor(
    ...
    fun getDirecTerm() {
        viewModelScope.launch {
            ...
            if (token.isNullOrEmpty()) {
                Log.e("DirecViewModel", "getDirecTerm: 토큰 없음")
                return@launch
            }
            try {
                val response = apiService.getDiercTerm(token)
                _direcTermData.value = response
                Log.d("DirecViewModel", "getDirecTerm: 저장목록 용어 서버 통신 성공")
            } catch (e: Exception) {
                Log.e("DirecViewModel", "getDirecTerm: 저장목록 용어 오류", e)
            }
        }
    }
    ...
    fun getDirecGpse() {
        viewModelScope.launch {
            val token = preferencesManager.getToken()
            if (token.isNullOrEmpty()) {
                Log.e("DirecViewModel", "getDirecGpse: 토큰 없음")
                return@launch
            }
            try {
                val response = apiService.getDiercGpse(token)
                _direcGpseData.value = response
                Log.d("DirecViewModel", "getDirecGpse: 저장목록 포도씨 서버 통신 성공")
            } catch (e: Exception) {
                Log.e("DirecViewModel", "getDirecGpse: 저장목록 포도씨 오류", e)
            }
        }
    }
    ...
    fun getDirecGps() {
        viewModelScope.launch {
            val token = preferencesManager.getToken()
            if (token.isNullOrEmpty()) {
                Log.e("DirecViewModel", "getDirecGps: 토큰 없음")
                return@launch
            }
            try {
                val response = apiService.getDiercGps(token)
                _direcGpsData.value = response
                Log.d("DirecViewModel", "getDirecGps: 저장목록 포도알 서버 통신 성공")
            } catch (e: Exception) {
                Log.e("DirecViewModel", "getDirecGps: 저장목록 포도알 오류", e)
            }
        }
    }
    ...
    fun getDirecGp() {
        viewModelScope.launch {
            val token = preferencesManager.getToken()
            if (token.isNullOrEmpty()) {
                Log.e("DirecViewModel", "getDirecGp: 토큰 없음")
                return@launch
            }
            try {
                val response = apiService.getDierctGp(token)
                _direcGpData.value = response
                Log.d("DirecViewModel", "getDirecGp: 저장목록 포도송이 서버 통신 성공")
                println("Gp임 $response")
            } catch (e: Exception) {
                Log.e("DirecViewModel", "getDirecGp: 저장목록 포도송이 오류", e)
            }
        }
    }
}
